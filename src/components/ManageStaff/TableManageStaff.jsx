...
export default function TableManageStaff({
  ...
  const lockPatientHandler = (e) => {
    Swal.fire({
      ...
      cancelButtonColor: 'green',
      cancelButtonText: 'انصراف',
      confirmButtonText: 'تحریم کارمند',
    }).then((result) => {
      if (result.isConfirmed) {
        setIsLoading(true);
        const data = new FormData();
        ...
        axios
          ...
          .then((res) => {
            ...
            Toast.fire({
              icon: 'success',
              text: 'کارمند مورد نظر با موفقیت تحریم شد',
            });
          })
          .catch((err) => {
            setIsLoading(false);
  ...
  const unLockPatientHandler = (e) => {
    Swal.fire({
      ...
      cancelButtonText: 'انصراف',
      confirmButtonText: 'رفع تحریم کارمند',
    }).then((result) => {
      if (result.isConfirmed) {
        setIsLoading(true);
        const data = new FormData();
        ...
        axios
          ...
          .then((res) => {
            ...
            Toast.fire({
              icon: 'success',
              text: 'کارمند مورد نظر با موفقیت رفع تحریم شد',
            });
          })
          .catch((err) => {
            setIsLoading(false);
                ...
                <TableRow key={staff.staffId}>
                  ...
                  <TableCell align="center">
                    <div className="flex justify-around">
                      ...
                      {!staff.isLocked && (
                        <Tooltip title="تحریم" placement="bottom">
                          <IconButton onClick={() => lockPatientHandler(staff)}>
                            <FaLockOpen className="text-emerald-500 text-lg" />
                          </IconButton>
                        </Tooltip>
                      )}
                      {staff.isLocked && (
                        <Tooltip title="رفع تحریم" placement="bottom">
                          <IconButton onClick={() => unLockPatientHandler(staff)}>
