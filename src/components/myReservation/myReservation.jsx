...
export default function MyReservation({ setPageState, account }) {
  const [pageStateMyReserv, setPageStateMyReserv] = useState(0);
      ...
      {pageStateMyReserv === 0 && (
        <div>
          <div className="text-start">
            <h3 className="text-xl font-bold">نوبت های {setPageState ? 'بیمار' : 'من'}</h3>
          </div>
          {reserveList.filter((e) => e.statusId === 1 || e.statusId === 3).length === 0 && !isLoading && (
            <div className="w-full mx-auto border rounded-lg mt-5 pt-2 pb-6">
          ...
          {reserveList
            .filter((e) => e.statusId === 1 || e.statusId === 3)
            .map((list) => (
              <div
                key={list.reservationId}
                style={{ backgroundColor: themeMode === 'light' ? 'rgb(248 250 252)' : '' }}
                className="w-full mx-auto border rounded-lg mt-5 py-2"
              >
                <MyReserveBox
                  list={list}
                  doctor={doctor}
          ...
          {visitedTime.length === 0 && !isLoading && (
            <div>
              <div className="flex justify-center">
                <svg width="136" height="136" viewBox="0 0 136 136" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M30.6374 52.3822L68.898 61.5235L54.503 76.3781L17 64.1897L30.6374 52.3822Z" fill="#EDF2F6" />
          ...
          {visitedTime.length > 0 && (
            <div className="flex w-full mx-auto flex-wrap">
              {visitedTime.map((list) => (
                <div key={list.appointmentId} className="lg:w-1/3 sm:w-1/2 w-full mt-1 px-1">
