...
export default function TableManageDoctor({
      ...
      if (result.isConfirmed) {
        ...
        axios
          ...
          .then((res) => {
            ...
            Toast.fire({
              icon: 'success',
              text: 'پزشک مورد نظر با موفقیت تحریم شد',
            });
          })
          .catch((err) => {
            setIsLoading(false);
  ...
  const unLockPatientHandler = (e) => {
    Swal.fire({
      ...
      cancelButtonText: 'انصراف',
      confirmButtonText: 'رفع تحریم پزشک',
    }).then((result) => {
      if (result.isConfirmed) {
        setIsLoading(true);
        const data = new FormData();
        ...
        axios
          ...
          .then((res) => {
            ...
            Toast.fire({
              icon: 'success',
              text: 'پزشک مورد نظر با موفقیت رفع تحریم شد',
            });
          })
          .catch((err) => {
            setIsLoading(false);
                    ...
                    <div className="flex justify-around">
                      ...
                      {!doctor.isLocked && (
                        <Tooltip title="تحریم" placement="bottom">
                          <IconButton
                          onClick={()=>lockPatientHandler(doctor)}
                          >
                      ...
                      {doctor.isLocked && (
                        <Tooltip title="رفع تحریم" placement="bottom">
                          <IconButton
                          onClick={()=> unLockPatientHandler(doctor)}
                          >
