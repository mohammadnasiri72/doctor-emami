...
export default function MainPageVisitHistory({ account, changeStatePages }) {
  ...
  useEffect(() => {
    if (pageStateVisitHistory === 0) {
      if (url !== '/dashboard/historyVisit') {
        navigate('/dashboard/historyVisit');
      }
    }
    if (pageStateVisitHistory === 1) {
      if (url !== '/dashboard/historyVisit/details') {
        navigate('/dashboard/historyVisit/details');
      }
    }
  }, [pageStateVisitHistory]);
          ...
          <div className="flex flex-wrap px-1">
            {historyReception.length > 0 &&
              historyReception.map((e) => (
                <div key={e.appointmentId} className="lg:w-1/3 sm:w-1/2 w-full mt-3 px-1">
                  <BoxReceptionPatient
                    reception={e}
