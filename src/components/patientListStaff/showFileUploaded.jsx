...
import BoxImg from '../UploadDocument/BoxImg';
import SimpleBackdrop from '../backdrop';

export default function ShowFileUploaded({ isShowfilePatient, setIsShowfilePatient, nationalId, appointmentId }) {
  ...
  const Toast = Swal.mixin({
    ...
    customClass: 'toast-modal-file',
  });

  // get list doctor
  useEffect(() => {
    if (isShowfilePatient) {
  ...
  useEffect(() => {
    if (nationalId && isShowfilePatient && doctor) {
  ...
  }, [nationalId, flag, isShowfilePatient, doctor]);
  ...
  //   move to appointmentId
  const moveToAppointmentId = (e) => {
    Swal.fire({
      title: 'انتقال فایل',
      text: 'آیا از انتقال فایل مطمئن هستید؟',
      showCancelButton: true,
      confirmButtonColor: 'green',
      cancelButtonColor: 'rgb(20 184 166)',
      cancelButtonText: 'انصراف',
      confirmButtonText: 'انتقال ',

      customClass: {
        container: 'toast-modal-file',
      },
    }).then((result) => {
      if (result.isConfirmed) {
        ...
        data.append('id', e.id);
        data.append('toAppointmentId', appointmentId);
        axios
          .post(`${mainDomain}/api/PatientFile/Move`, data, {
            headers: {
              Authorization: `Bearer ${localStorage.getItem('token')}`,
            },
          })
  ...
  return (
    <>
      <div
        style={{
          zIndex: '1303',
          transform: isShowfilePatient ? 'translateX(0)' : 'translateX(-100%)',
            ...
            <Table aria-label="sticky table">
              <TableHead className="">
                <TableRow>
                  ...
                  {appointmentId && <TableCell align="center">انتقال به پذیرش</TableCell>}
                </TableRow>
              </TableHead>
              <TableBody>
                {listFilePatient.map((e, i) => (
                  <TableRow key={i}>
                    ...
                    {appointmentId && (
                      <TableCell align="center">
                        <span onClick={() => moveToAppointmentId(e)} className="text-teal-500 cursor-pointer">
                          انتقال
                        </span>
                      </TableCell>
                    )}
